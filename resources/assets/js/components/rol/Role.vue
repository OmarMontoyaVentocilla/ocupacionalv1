                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <div class="row">
                            ...
                            <div class="col-lg-4">
                                <select class="form-control input-sm" v-model="combo" @change="readRoles()">
                            ...
                            <div class="col-lg-4">
                                    <input type="text" class="form-control" placeholder="Buscar" v-model="filter" @keyup="readRoles()">
                            </div>  

                            <div class="col-lg-4">
                                    <button @click.prevent="AddRole()" class="btn btn-primary">+ Nuevo Rol</button>   
                            </div>
                                    ...
                                    <table class="table table-bordered table-striped" style="background:white;">
                                        ...
                                        <tbody>    
                                            ...
                                            <td>
                                                ...
                                                <button class="btn btn-danger  btn-sm"  @click.prevent="deleteRol(list)">Borrar</button>
                                            </td>
                                            </tr>
                                        </tbody>
                            </table>
...
<li v-if="pagination.current_page < pagination.last_page">
      ...
      <div class="col-lg-3 col-xs-8">
       ...
       <button class="btn btn-default" v-on:click.prevent="readRoles()"><i class="fa fa-spinner"></i></button>
      </div>  
        ...
        <div class="modal fade" tabindex="-1" role="dialog" id="add_rol_model">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                  ...
                  <div v-bind:class="{'form-group':true,'has-error':error.name}">
                        ...
                        <input type="text" name="name" id="name" placeholder="Nombre de rol" class="form-control" v-model="rol.name">
                  </div>
                   <span class="help-block text-danger" v-for="error_x in error.name">{{error_x}}</span>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                        <button type="button" @click.prevent="createRol" class="btn btn-primary" id="envio_grabar">Grabar</button>
                    </div>
                </div>
            </div>
        </div>
         ...
         <div class="modal fade" tabindex="-1" role="dialog" id="update_rol_model">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                    ...
                    <div class="modal-body">
                        <div v-bind:class="{'form-group':true,'has-error':error_update.name}">
                            <label>Nombre:</label>
                            <input type="text" placeholder="Nombre de rol" class="form-control"  v-model="rol.update_rol">
                        </div>
                         <span class="help-block text-danger" v-for="error_role in error_update.name">{{error_role}}</span>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                        <button type="button" @click.prevent="updateRol(rol.id)" class="btn btn-primary" id="envio_actualizar">Guardar</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            initUpdate(index){
                this.errors_x = [];
                this.error_update=[];
                $("#update_rol_model").modal("show");
                $('body').css('padding-right','0px');
                this.rol.update_rol = index.name;
                this.rol.id=index.id;
            },
            updateRol(info_id){
               axios.patch('/roles/' + info_id, {
                    name: this.rol.update_rol
                })
                    .then(response => {
                        this.mensaje('Excelente',response.data.message,'success');
                        $("#update_rol_model").modal("hide");
                        this.readRoles();
          ...
          swal({
            ...
            readRoles(page){
                ...
                let url_roles='/listaroles?page='+page;
                axios.get(url_roles,{ params: { valor_combo: valor_combo,valor_caja:valor_caja}})
                    .then(response => {
                         var arreglo=[
                        this.lista=response.data.roles.data,
            ...
            createRol(){
                var sel_thi=this;
                sel_thi.disabl(true,'envio_grabar');
                setTimeout(function(){sel_thi.disabl(false,'envio_grabar');}, 3000);
                axios.post('/roles', {name: this.rol.name})
                    .then(response => {
                        this.error_update=[];
                    	this.mensaje('Excelente',response.data.message,'success');
                        this.reset();
                        $("#add_rol_model").modal("hide");
                        this.readRoles();
                    })
                    .catch(error => {
