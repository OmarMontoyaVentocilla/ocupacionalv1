       ...
       <button type="button" id="shows" :class="descripcionColor" v-on:click="mostrar=!mostrar">
        ...
        <template v-else><i class="fa fa-times"></i></template>
          ...
          <div class="col-lg-3 col-md-4 col-sm-5 col-xs-5">
           <select class="form-control input-sm" v-model="combo" @change="getIdioma()">
                 <option value="">Seleccione</option>
                 <option v-for="sel in seleccion" :value="sel.id">{{sel.nombre}}</option>
           </select>
          </div>
           <div class="col-lg-3 col-md-4 col-sm-4 col-xs-5">
            <input type="text" class="form-control" placeholder="Buscar" v-model="filter" @keyup="getIdioma()">
          </div> 
  </div><br>
<template v-if="mostrar">
  <div class="row well agregar_idioma" >
      <div class="col-lg-12">
        <form v-on:submit.prevent="addIdioma()">
          <div class="form-group row">
          <div class="col-lg-4"> 
    <label for="nombre_idioma" class="etiqueta"> Nombre:</label>
<input v-validate="'required'" :class="{'form-control': true, 'is-danger': errors.has('nombre_idioma') }" type="text" id="nombre_idioma" v-model="idioma.nombre_idioma" name="nombre_idioma">
<span  v-show="errors.has('nombre_idioma')" class="help is-danger">{{ errors.first('nombre_idioma') }}</span>
          </div>
          </div>  
           <button type="submit" class="btn btn-primary mb-2" id="envio2">Guardar</button> 
        </form>
      </div>
  </div>
</template>
...
<modal>
  <h3 slot="header" class="modal-title">{{titulo}}</h3>
   ...
   <div slot="body">
   <form v-on:submit.prevent="editIdioma(idioma.id)">
     <div class="form-group">
      <label for="nombre_idioma_edit">Nombre:</label>
     <input type="text" class="form-control" id="nombre_idioma_edit" v-model="idioma.nombre_idioma_edit">
    </div>
  ...
  <div class="row">
    ...
    <table class="table table-bordered table-hover table-fixed table-striped" style="background:white;">
         ...
         <tr v-for="(list,index) in lista">
           <td >{{index+1}}</td>
           <td >{{list.nombre_idioma}}</td>
           <td>
      <button type="button" class="btn btn-info"   v-on:click.prevent="showIdioma(list)"><i class="fa fa-edit"></i> Editar</button>
      <button type="button" class="btn btn-danger" v-on:click.prevent="deleteIdioma(list)"><i class="fa fa-trash"></i> Eliminar</button>
           </td>
         </tr>
        </template>
        <template v-else>
     ...
     <div class="row">
      ...
      <div class="col-lg-3 col-xs-8">
       ...
       <button class="btn btn-default" v-on:click.prevent="getIdioma()"><i class="fa fa-spinner"></i></button>
      </div>     
...
export default{
   ...
   methods:{
     ...
     showIdioma(info){
       this.idioma.nombre_idioma_edit=info.nombre_idioma;
       this.idioma.id=info.id;
      $('#myModal2').modal('show');
      $('body').css('padding-right','0px');
     },
     editIdioma(info){
      var selfx = this;
      var nombre_idioma_edit = this.idioma.nombre_idioma_edit;
       axios.patch('idioma/' + info, {nombre_idioma_edit:nombre_idioma_edit}).then(response => {  
     ...
     disabl(valor){
        $("#envio2").prop('disabled', valor);
     },
     reset(){
        this.idioma.nombre_idioma='';
        this.idioma.nombre_idioma_edit='';
     },
     addIdioma(){
      var sel_thi=this;
      sel_thi.disabl(true);
      setTimeout(function(){sel_thi.disabl(false); }, 3000);
         let url_p = '/idioma';
            var idioma = this.idioma;
            axios.post(url_p, idioma)
                .then((response) => {
     ...
     deleteIdioma(info){
          let url_delete="idioma/"+info.id;
          var selfdel=this;
          swal({
                   title: 'Estas seguro?',
                   text: "Eliminara a "+info.nombre_idioma,
                   type: 'warning',
